      ...
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-slate-100">
          {stocks.map((stock) => {
            ...
            const average =
              totalBoughtShares > 0 ? investedWithComm / totalBoughtShares : 0;
            return (
              <tr
                key={stock.id}
              ...
              >
                ...
                <td className="py-3 pr-4 text-right text-slate-700 tabular-nums">
                  {heldShares > 0
                    ? heldShares.toLocaleString("en-BD", {
                ...
                <td className="py-3 pr-4 text-right text-slate-700 tabular-nums">
                  {investedWithComm > 0
                    ? formatCurrency(investedWithComm)
                    : "—"}
                </td>
                ...
                <td
                  className={`py-3 pr-4 text-right tabular-nums font-medium ${net >= 0 ? "text-emerald-600" : "text-red-600"}`}
                >
                  {investedWithComm > 0 || sells > 0 || dividends > 0
