...
export function stockTotals(stock: Stock) {
  const investedWithComm = stock.buys.reduce(
    (acc, b) => acc + Number(b.unitPrice) * Number(b.numberOfStocks) + Number(b.commission),
    0,
  );
  const investedWithoutComm = stock.buys.reduce(
  ...
  const net = sells + dividends - investedWithComm - purification;
  ...
  return { investedWithComm, investedWithoutComm, sells, dividends, purification, net, heldShares, totalBoughtShares, totalSoldShares };
}
